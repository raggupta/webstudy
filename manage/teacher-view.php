...
<tbody>
<?php
$sql = "Select * from member where member_type='Teacher' ";
...
if ( mysqli_num_rows( $row ) > 0 ) {
    while ( $res = mysqli_fetch_assoc( $row ) ) {
        ...
        <td>
        <i class = "mdi mdi-email"> </i> <?php echo $res['email'] ;
        ?>
        </td>
        <td>
        <i class = "mdi mdi-password"> </i> <?php echo $res['password'] ;
        ?>
        </td>
        <td>
        <span class = "text-gray">
        <label class = "badge <?php echo ($res['status'] == "Active") ? 'badge-success' : 'badge-danger'; ?>">
        <?php echo $res['status'];
        ?>
        </label>
        </td>
        <td>
        <i class = "mdi mdi-calendar"> </i> <?php echo $res['create_dt'] . "<br><i class = 'mdi mdi-clock'> </i> " . $res['create_tm'];
        ?>
        </td>

        <td class = "text-right">
        <a href = "teacher-add?id=<?php echo $res['id']; ?>">
        <div class = "btn btn-inverse-success has-icon btn-rounded">
        <i class = "mdi mdi-pencil-outline"></i>Edit
        </div>
        </a>
        <a href = "teacher-view?d_id=<?php echo $res['id']; ?>">
